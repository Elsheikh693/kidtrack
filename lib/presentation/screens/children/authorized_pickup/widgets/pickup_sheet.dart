...
class _PickupSheetState extends State<PickupSheet> with KeyboardSheetMixin {
  ...
  final _relationships = ['father', 'mother', 'grandfather', 'grandmother', 'uncle', 'aunt', 'driver', 'other'];
  ...
  @override
  void dispose() { nameCtrl.dispose(); phoneCtrl.dispose(); idCtrl.dispose(); super.dispose(); }
  ...
  Future<void> _submit() async {
    if (_childId == null) { Loader.showError('pickup_error_child'.tr); return; }
    final name = nameCtrl.text.trim();
    if (name.isEmpty) { Loader.showError('pickup_error_name'.tr); return; }
    ...
    final item = AuthorizedPickupModel(
      key: id, nurseryId: nurseryId, childId: _childId!, name: name,
    ...
    if (isEdit) {
      await _service.update(item: item, callBack: (s) {
        Loader.dismiss();
        if (s == ResponseStatus.success) { Loader.showSuccess('pickup_success_updated'.tr); Get.back(); }
        else Loader.showError('pickup_error_failed'.tr);
      });
    } else {
      await _service.add(item: item, callBack: (s) {
        Loader.dismiss();
        if (s == ResponseStatus.success) { Loader.showSuccess('pickup_success_added'.tr); Get.back(); }
        else Loader.showError('pickup_error_failed'.tr);
      });
    }
  }
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(isEdit ? 'pickup_edit_title'.tr : 'pickup_add_title'.tr, style: context.typography.mdBold.copyWith(fontSize: 18, color: const Color(0xFF1E293B))),
              ...
              if (_loading) const Center(child: CircularProgressIndicator()) else ...[
                ...
                DropdownButtonFormField<ChildModel>(
                  ...
                  hint: Text('pickup_child_hint'.tr, style: context.typography.smRegular.copyWith(color: const Color(0xFFCBD5E1), fontSize: 14)),
                ...
                DropdownButtonFormField<String>(
                  ...
                  items: _relationships.map((r) => DropdownMenuItem(value: r, child: Text(_relLabel(r)))).toList(),
                  decoration: _dec(),
                ),
                SizedBox(height: 16.h),
                _Label('pickup_phone_label'.tr),
                SizedBox(height: 6.h),
                _Field(controller: phoneCtrl, hint: 'pickup_phone_hint'.tr, keyboardType: TextInputType.phone),
                SizedBox(height: 16.h),
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: _submit,
                    style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFF59E0B), foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14.r)), elevation: 0),
                    child: Text('pickup_save'.tr, style: context.typography.smSemiBold.copyWith(fontSize: 16)),
                  ),
                ),
              ],
            ],
...
class _Field extends StatelessWidget {
  ...
  Widget build(BuildContext context) => TextField(
    controller: controller, keyboardType: keyboardType, maxLines: 1,
