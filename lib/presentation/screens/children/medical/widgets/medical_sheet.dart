...
class _MedicalSheetState extends State<MedicalSheet> with KeyboardSheetMixin {
  ...
  Future<void> _submit() async {
    if (_childId == null) { Loader.showError('medical_error_child'.tr); return; }
    ...
    final item = MedicalProfileModel(
      key: id, childId: _childId!, nurseryId: nurseryId,
      allergies: _allergies, medications: _medications,
      bloodType: _bloodType,
      emergencyContact: emergencyContactCtrl.text.trim().isEmpty ? null : emergencyContactCtrl.text.trim(),
      emergencyPhone: emergencyPhoneCtrl.text.trim().isEmpty ? null : emergencyPhoneCtrl.text.trim(),
    ...
    if (isEdit) {
      await _service.update(item: item, callBack: (s) {
        Loader.dismiss();
        if (s == ResponseStatus.success) { Loader.showSuccess('medical_success_updated'.tr); Get.back(); }
        else Loader.showError('medical_error_failed'.tr);
      });
    } else {
      await _service.add(item: item, callBack: (s) {
        Loader.dismiss();
        if (s == ResponseStatus.success) { Loader.showSuccess('medical_success_added'.tr); Get.back(); }
        else Loader.showError('medical_error_failed'.tr);
      });
    }
  }
  ...
  void _addAllergy() {
    ...
    if (v.isNotEmpty && !_allergies.contains(v)) {
      setState(() { _allergies.add(v); allergyCtrl.clear(); });
    }
  }
        ...
        child: SingleChildScrollView(
          ...
          child: Column(
            ...
            children: [
              ...
              Text(isEdit ? 'medical_edit_title'.tr : 'medical_add_title'.tr, style: context.typography.mdBold.copyWith(fontSize: 18, color: const Color(0xFF1E293B))),
              ...
              if (_loading) const Center(child: CircularProgressIndicator()) else ...[
                ...
                DropdownButtonFormField<ChildModel>(
                  ...
                  hint: Text('medical_child_hint'.tr, style: context.typography.smRegular.copyWith(color: const Color(0xFFCBD5E1), fontSize: 14)),
                ...
                DropdownButtonFormField<String>(
                  value: _bloodType,
                  hint: Text('medical_blood_type_hint'.tr, style: context.typography.smRegular.copyWith(color: const Color(0xFFCBD5E1), fontSize: 14)),
                  onChanged: (v) => setState(() => _bloodType = v),
                  items: ['A+','A-','B+','B-','O+','O-','AB+','AB-'].map((t) => DropdownMenuItem(value: t, child: Text(t))).toList(),
                  decoration: _dropDecoration(),
                ),
                SizedBox(height: 16.h),
                ...
                SizedBox(height: 6.h),
                Row(children: [
                  Expanded(child: _Field(controller: allergyCtrl, hint: 'medical_allergy_hint'.tr)),
                  SizedBox(width: 8.w),
                  IconButton(onPressed: _addAllergy, icon: const Icon(Icons.add_circle_outline, color: Color(0xFFEF4444))),
                ]),
                if (_allergies.isNotEmpty) Wrap(spacing: 6.w, runSpacing: 4.h, children: _allergies.map((a) => Chip(
                  label: Text(a, style: context.typography.xsRegular.copyWith(fontSize: 12)),
                ...
                SizedBox(height: 6.h),
                _Field(controller: emergencyContactCtrl, hint: 'medical_emergency_contact_hint'.tr),
                SizedBox(height: 12.h),
                _Field(controller: emergencyPhoneCtrl, hint: 'medical_emergency_phone_hint'.tr, keyboardType: TextInputType.phone),
                SizedBox(height: 16.h),
                _Label('medical_doctor_label'.tr),
                SizedBox(height: 6.h),
                _Field(controller: doctorCtrl, hint: 'medical_doctor_hint'.tr),
                SizedBox(height: 12.h),
                _Field(controller: doctorPhoneCtrl, hint: 'medical_doctor_phone_hint'.tr, keyboardType: TextInputType.phone),
                SizedBox(height: 16.h),
                _Label('medical_notes_label'.tr),
                SizedBox(height: 6.h),
                _Field(controller: notesCtrl, hint: 'medical_notes_hint'.tr, maxLines: 3),
                SizedBox(height: 32.h),
                SizedBox(
                  width: double.infinity, height: 52.h,
                  child: ElevatedButton(
                    onPressed: _submit,
                    style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFEF4444), foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14.r)), elevation: 0),
                    child: Text('medical_save'.tr, style: context.typography.smSemiBold.copyWith(fontSize: 16)),
                  ),
                ),
              ],
            ],
